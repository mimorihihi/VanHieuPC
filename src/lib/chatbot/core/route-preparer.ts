...
export function prepareToolRoute(route: ToolRoute | null, message: string): ToolRoute | null {
  if (!route) return null

  const recommendationIntent = extractRecommendationIntent(message)
  const productKeyword = extractProductKeyword(message) || message
  ...
  return {
    ...
    params: {
      ...
      minPrice: route.params?.minPrice ?? (route.toolName === "recommendProducts" ? recommendationIntent.minPrice : undefined),
      maxPrice: route.params?.maxPrice ?? (route.toolName === "recommendProducts" ? recommendationIntent.maxPrice : undefined),
    },
  }
}
